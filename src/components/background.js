...
const Background = ({ layoutRef }) => {
	...
	const scrollDownBgArray = () => setBgArray((prevArray) => {
	...
	const scrollUpBgArray = () => setBgArray((prevArray) => {
		const newArray = new Array(prevArray.length);
	...
	useEffect(() => {
		...
		const handleScroll = (e) => {
			...
			if (element.scrollTop >= 0.75 * element.scrollHeight && lastScroll.current <= 0.75 * element.scrollHeight) {
				scrollDownBgArray();
				neededScroll.current = -element.scrollHeight / bgArray.length;
			} else if (element.scrollTop <= 0.25 * element.scrollHeight && lastScroll.current >= 0.25 * element.scrollHeight) {
				scrollUpBgArray();
				neededScroll.current = element.scrollHeight / bgArray.length;
			}
	...
	return (
		<div ref={ref} className="absolute top-0 left-0 w-full h-full -z-10 overflow-hidden">
			{bgArray.map((image) => (
				<div className="w-screen h-screen" style={{ backgroundImage: `url(${image})`, backgroundPosition: "center", backgroundSize: "cover" }} />
			))}
		</div>
	);
}
